...
module.exports = function( handlebars ) {
	return function( config ) {
		...
		config = _.defaults( config, {
			...
			// source:
			// (string) directory in which to place translated files (optional)
			dest: '',
			...
			outputExtension: '.tmpl'
		} );

		if(config.outputExtension[0] !== '.') {
			config.outputExtension = '.' + config.outputExtension;
		}
		...
		var partialTemplates = _.filter( sourceFiles, determineIfPartial );
		...
		return {
			exportify: function( options ) {
				...
				// TODO: Clean up API, what's really needed here?
				var defaults = {
					source: '',
					dest: '',
					model: {},
				...
				_.each( toExport, function( paths ) {
					...
					var translated = '';
					try {
						translated = config.translator.translate( paths.source, util.file.read( paths.source ), paths.translateOpts, paths.model );
					} catch ( exception ) {
		...
		function determineIfPartial( filePath ) {
			return config.isPartial( getPathFromSourceDir( filePath ) );
		}
